...
- **Large float comparisons now respect relative precision.** Comparing a
  recalculated figure against a file value previously used a rigid absolute
  `1e-9` tolerance. On sums over large ranges (thousands or hundreds of thousands
  of cells) reaching millions or billions, normal IEEE 754 floating-point
  roundoff between formula engines caused identical displayed figures (e.g.
  `4,171,793,037.02`) to be reported as divergent. Numeric comparisons now use
  `math.isclose` with relative and absolute tolerances (`rel_tol=1e-9`,
  `abs_tol=1e-9`), and numerical agreement reports `same`.
- **Sheet relationship resolution handles attribute order and path variations.**
  `_error_cached_values` previously assumed `Id` preceded `Target` in
  `workbook.xml.rels` and failed on workbooks generated by openpyxl or LibreOffice
  where attribute order differed or target paths contained leading slashes.
  Relationships are now extracted with an attribute-order agnostic parser.

## [1.5.1] — 2026-09-06

### Fixed

- **Cached formula errors are now retained when the fast reader omits them.**
  A formula already saved by Excel with an error such as `#DIV/0!` could be
  reported as *Not stored*, even though the workbook contained the cached
  result. The loader now recovers those error values so the report shows the
  value Excel saved.
