...
formula_pattern_coverage gives total/shown/omitted graph formula nodes. Omitted patterns do not establish missing formulas or cached values. Use external_workbooks to describe referenced external files: unread workbooks were not opened from disk; dependent cells use embedded file caches or have no value. A known cached value does not prove an external file was opened or read. When explaining intervals or constraints, state both lower and upper bounds explicitly without omitting the lower bound.
