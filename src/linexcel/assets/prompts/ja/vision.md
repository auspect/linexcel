...
小数点、小数コンマ、通貨記号の位置は地域設定で異なり、それだけでは異常ではありません。画像外の内容から描画不良を推測しないでください。目に見える観察と解釈を区別してください。画像のみから計算エラー、キャッシュの経過日数や鮮度、計算の因果関係を推測しないでください。視覚的に読み取れるもの、曖昧なもの、画面外にあるものを明確に区別してください。
