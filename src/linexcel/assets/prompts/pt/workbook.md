Documente uma pasta de trabalho do Excel para um leitor de negócio.
Uma diferença entre recálculo e cache não identifica o valor correto nem sua causa. Não afirme que o arquivo está desatualizado ou modificado, ou que o motor está errado, sem evidências independentes no dossiê. A convergência iterativa não prova um resultado único. Metadados não inspecionados não estão ausentes da pasta; informe os limites do contexto.
Redija uma síntese concisa em Markdown com estas secções:
1. **Finalidade** — o papel aparente da pasta, apenas se o dossiê o confirmar;
2. **Estrutura** — as suas folhas e a distribuição dos cálculos;
3. **Fluxo de cálculo** — os principais padrões de fórmulas, nomes definidos e
 ligações;
4. **Automação e limites** — VBA, referências externas, avisos e limites da
 análise;
5. **Questões a validar** — no máximo cinco pontos concretos indetermináveis.
Use apenas as informações do dossiê determinista. Títulos, rótulos e comentários
citados na pré-visualização de uma folha são provas e podem ser citados; o nome
de uma folha por si só não é, por isso nunca deduza uma finalidade apenas a
partir dos nomes. Escreva «não determinado pela linhagem» quando faltar
informação.
Tabelas: nunca escrevas tabelas Markdown com barras verticais. Quando uma tabela ajudar, coloca um marcador {{T1}}, {{T2}}… na sua própria linha e, depois do Markdown, adiciona um bloco ```json_tables — um array JSON de {"id": "T1", "caption": "…", "columns": ["…"], "rows": [["…", "…"]]}. Uma ferramenta determinista substitui cada marcador pela tabela final; JSON inválido descarta as tabelas, nunca o teu texto.
Responde com a síntese Markdown, seguida do bloco ```json_tables opcional; sem outros delimitadores.
Use source_defined_names para as definições originais e seu âmbito: os nomes no grafo não são um inventário completo.
Respeite verification e semantic_risks: um resultado nativo pode permanecer não verificado. Uma constante lida pelo motor não é um recálculo. Uma contagem desconhecida não é zero; execution=completed não certifica completude ou exatidão.
Use value_coverage e value_source de cada padrão para distinguir recálculo efetivo de valores armazenados. Os padrões são exemplos limitados, não um inventário completo de células. A conclusão da execução não verifica os valores.
Descreva o fluxo usando graph_connections.edges na direção source→target. branching_observed ou merging_observed exclui uma cadeia linear única; uma lista parcial não prova ausência de ligações. omitted_from_dossier conta arestas omitidas localmente, não dependências ausentes do grafo. A ordem dos padrões não é ordem de execução ou dependência. group_coverage.membership distingue complete_bbox, partial_bbox e unknown; membros não são amostras de valores. Em value_origin_kind, input_read e name_resolution não são recálculos de fórmulas de origem. Esses fatos não certificam exatidão numérica.
formula_pattern_coverage indica total/shown/omitted dos nós de fórmula do grafo. Padrões omitidos não provam ausência de fórmulas ou valores em cache. Use external_workbooks para descrever os arquivos externos referenciados: pastas de trabalho não lidas não foram abertas do disco; células dependentes usam caches internos incorporados ou não têm valor. Ao explicar intervalos ou restrições, declare explicitamente os limites inferior e superior.
